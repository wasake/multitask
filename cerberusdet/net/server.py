...
class Server:
    def __init__(
        ...
        port: int = 5000,
        rounds: int = 50,
        server_trains: bool = True  # 服务器是否参与训练的选项
    ):
    ...
    def train_model(self, round_num, resume_optimizer=False):
        ...
        # 准备训练参数 - 从配置中获取而非硬编码
        batch_size = self.server_config.get("batch_size", 16)  # 使用配置或默认值
        imgsz = self.server_config.get("imgsz", 640)  # 使用配置或默认值
    ...
    def train_and_aggregate(self):
        ...
        for round_num in range(self.rounds):
            ...
            for i, conn in enumerate(self.clients):
                logging.info(f"等待客户端 {i+1} 发送训练后的模型参数...")
                data = self.receive_data(conn)
                if data and "model_params" in data:
                    client_params.append(data["model_params"])
                    logging.info(f"成功接收客户端 {i+1} 的模型参数")
                else:
                    logging.error(f"未能接收客户端 {i+1} 的模型参数")
            ...
            # 发送聚合后的模型参数给客户端
            logging.info("将聚合后的模型参数发送给所有客户端...")
            for i, conn in enumerate(self.clients):
                self.send_data(conn, {"aggregated_model_params": aggregated_params})
                logging.info(f"已发送聚合后的模型参数到客户端 {i+1}")
    ...
    def aggregate_models(self, client_params, server_params=None):
        ...
        return averaged_params  # 只返回backbone部分的聚合参数
...
if __name__ == "__main__":
    ...
    server = Server(
        ...
        port=5000,
        rounds=50,
        server_trains=True  # 设置服务器是否参与训练
    )
    server.start()