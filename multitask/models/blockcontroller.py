...
def concat(pt1, pt2):
    # 加载两个YOLO模型
    model1 = torch.load("yolo1.pt")["model"]
    model2 = torch.load("yolo2.pt")["model"]

    # 提取组件（假设模型是nn.Module类型，结构与YOLO一致）
    backbone1 = model1.model[:8]  # 假设backbone在前8层
    neck1 = model1.model[8:20]    # 假设neck在中间层
    head1 = model1.model[20:]     # 假设head在最后层
    neck2 = model2.model[8:20]
    head2 = model2.model[20:]

    # 初始化CerberusDet
    task_ids = ["task1", "task2"]  # 定义两个任务
    cerberus_model = CerberusDet(task_ids=task_ids, nc=80, cfg="yolov5s.yaml", verbose=True)

    # 添加backbone
    backbone_controller = cerberus_model.add_block(backbone1)

    # 添加第一个分支的neck和head
    neck1_controller = cerberus_model.add_block(neck1)
    neck1_controller.stack_on(backbone_controller)  # 连接到backbone
    head1_controller = cerberus_model.add_head(head1, "task1")
    head1_controller.stack_on(neck1_controller)     # 连接到neck1

    # 添加第二个分支的neck和head
    neck2_controller = cerberus_model.add_block(neck2)
    neck2_controller.stack_on(backbone_controller)  # 连接到backbone
    head2_controller = cerberus_model.add_head(head2, "task2")
    head2_controller.stack_on(neck2_controller)     # 连接到neck2

    # 生成执行计划
    exec_order, branching_ids = cerberus_model.execution_plan(task_ids)
    print("Execution order:", exec_order)
    print("Branching points:", branching_ids)

    # 测试前向传播
    input_tensor = torch.randn(1, 3, 640, 640)  # 示例输入
    outputs = cerberus_model.forward(input_tensor, task_ids)
    print("Task1 output:", outputs["task1"])
    print("Task2 output:", outputs["task2"])


def unconcat(pt, indexs):
    pass
