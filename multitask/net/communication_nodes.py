...
import os
import shutil
from multitask.models.yolomanager import YOLOManager  # 假设 yolomanager.py 在此路径

class NetworkEndpoint():
    def __init__(self, pt_path: str, data_yaml: str, hyp: str, device: str = "cuda"):
        """
        初始化通讯节点。
...
class Client(NetworkEndpoint):
    def __init__(self, server, pt_path: str, data_yaml: str, hyp: str, device: str = "cuda"):
        """
        初始化客户端。
        ...
        参数:
            ...
            device (str): 运行设备，默认为 "cuda"
        """
        super().__init__(pt_path, data_yaml, hyp, device)
        self.server = server
    ...
    def transmit(self, epochs: int = 1):
        """
        训练并与服务器通信。
...
class Server(NetworkEndpoint):
    ...
    def aggregate(self, client_pt_path: str):
        ...
        """
        # 加载客户端模型
        client_model = YOLO(client_pt_path)
        client_backbone_params = self.yolo_manager.get_parameters(part="backbone")  # 使用 YOLOManager 获取 backbone 参数
